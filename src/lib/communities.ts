...
export const COMMUNITIES: Record<string, CommunityConfig> = {
  sagkeeng: {
    slug: "sagkeeng",
    name: "Sagkeeng Anicinabe Nation",
    tagline: "Zaagiing — Mouth of the River",
...
export function getCommunityConfig(slug: string, dbCommunity?: { name: string; tagline: string; crestUrl?: string | null }): CommunityConfig {
  ...
  if (fromCode) {
    if (dbCommunity) return { ...fromCode, name: dbCommunity.name || fromCode.name, tagline: dbCommunity.tagline || fromCode.tagline, crestUrl: dbCommunity.crestUrl || fromCode.crestUrl };
    return fromCode;
  }
