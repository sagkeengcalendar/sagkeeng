...
export async function sendEmail({ to, subject, body, from }: SendEmailArgs): Promise<EmailResult> {
  ...
  if (!apiKey) {
    console.log("\n── EMAIL (DEMO — set RESEND_API_KEY) ──");
    console.log("  To:", to, "| Subject:", subject);
    console.log("──────────────────────────────────────\n");
    return { status: "demo", to, subject, body };
  }
  ...
  try {
    ...
    if (!res.ok) return { status: "failed", error: data?.message || `Resend error ${res.status}`, to, subject, body };
    return { status: "sent", to, subject, body };
  } catch (err) {
    return { status: "failed", error: err instanceof Error ? err.message : String(err), to, subject, body };
  }
}
